...
def cost_function_prime(y_true, y_pred):
    ...
    cost_prime = -(y_true - y_pred)#*activation(y_pred)
    return cost_prime
...
class NeuralNetwork(object):
    ...
    def forward(self, input):
        ...
        """
        The 'zip' function is used basically to get the values at 
        some particular indeces of 
        the weight and biases matrices and use those particular values to
        produce the activaation of the neuron. For example, weights[0][0] 
        give the weights the 1st neuron in the input layer produces for all
        the neurons in the next layes; similarly, the biases[0][0]
        give us the bias values of the the 1st layer after the input layer.
        The notation of all these terms in terms of indeces of the matrices
        is a bit tiring, so instead we use the 'zip' function which makes our 
        life easy.'
        """
    ...
    def compute_deltas(self, pre_activations, y_true, y_pred):
        ...
        for l in range(len(deltas) - 2, -1, -1):
            ...
            delta[l] = np.dot(self.weights[l + 1].transpose(), delta[l + 1]) * activation(pre_activations[l], derivative=True)
            return deltas
    ...
    def backpropagate(self, deltas, pre_activations, activations):
        ...
        for l in range(1, len(self.size)):
            ...
            dW_temp = np.dot(deltas[1], activations[l-1].transpose())  ####There was an error in these two lines, which are rectified by analyzing the code in line 263 and 264
            dB_temp = deltas[l]
    ...
    def train(self, X, y, batch_size, epochs, learning_rate, validation_split=0.2, print_every=10):
        ...
        for e in epoch_iterator:
            ...
            """
            Enumerate function's main working is for the indexing of anything which is being iterated over & over and is used if it's difficult to keep track of the things which iterate to very large amount.
            """
            for i, (dw_epoch, db_epoch) in enumerate(zip(dw_per_epoch, db_per_epoch)):
                ...
                self.weights = self.weights[i] - learning_rate*dw_epoch
                self.biases = self.biases[i] - learning_rate*db_epoch
